...
install() {
    ...
    elif [ -d "${moddir}/../livekit-mos/bin" ]; then
        STATIC_BIN="${moddir}/../livekit-mos/bin"
    elif [ -d "${moddir}/../../livekit-mos/bin" ]; then
        STATIC_BIN="${moddir}/../../livekit-mos/bin"
    fi
    ...
    if [ -z "$STATIC_BIN" ] || [ ! -x "$STATIC_BIN/busybox" ]; then
        ...
        derror "  - /usr/share/minios-dracut/bin (from package)"
    ...
    elif [ -f "${moddir}/../livekit-mos/lib/livekitlib" ]; then
        LIVEKITLIB_SRC="${moddir}/../livekit-mos/lib/livekitlib"
    elif [ -f "${moddir}/../../livekit-mos/lib/livekitlib" ]; then
        LIVEKITLIB_SRC="${moddir}/../../livekit-mos/lib/livekitlib"
    ...
    if [ -z "$LIVEKITLIB_SRC" ] || [ ! -f "$LIVEKITLIB_SRC" ]; then
        ...
        derror "Expected locations:"
        derror "  - /usr/share/minios-dracut/lib/livekitlib (from package)"
